...
* _rotate_
  ...
  * For example, a box with a starting rotation of `0,0,0` is copied at a _rotate_ input of `0,0,45`. The first copy will be rotated by `0,0,45`, the second copy will be rotated another `0,0,45` for a total rotation of `0,0,90` (from the original input primitives), and so on.
...
* _scale_
  ...
  * For example, a box (with dimensions `1000,1000,1000`) is copied at a _scale_ input of `1,1,1.5`. The first copy will be scaled by `1,1,1.5` (now with dimensions of `1000,1000,1500`), the second copy will scale the first copy (that has the dimensions of `1000,1000,1500`) by `1,1,1.5` (now with dimensions of `1000,1000,2250`), and so on.
...
* _use iterator(s)_
  ...
  * Sets whether to use [**iterator**](/nodes/Iterator/documentation.md) nodes with this **copy** node.
...
* <a href="https://creator.trimble.com/graph?assetURI=whp:d62fe689-b7c0-4fd6-8d17-70deef935725&version=latest" target="_blank">Loft as NURBS patch</a>
