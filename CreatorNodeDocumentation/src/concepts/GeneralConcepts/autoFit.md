...
An AutoFit Live Component is a [Live Component](/concepts/GeneralConcepts/liveComponent.md)  featuring a **[geometry input](/nodes/GeometryInputV2/documentation.md)** node—it ingests SketchUp surfaces to automatically adapt its shape using a chosen face.

![](images\AutofitExample.gif)

### In Situ

When an AutoFit Live Component is applied to a face in SketchUp, that face is converted to JSON and passed into the [graph](/concepts/GeneralConcepts/graph.md) through its **[geometry input](/nodes/GeometryInputV2/documentation.md)** node. The graph then [computes](/concepts/GeneralConcepts/compute.md) its output utilizing this new geometry.
...
An AutoFit graph uses a **[geometry input](/nodes/GeometryInputV2/documentation.md)** node as one of its parametric inputs. 
...
- Measure it with nodes like **[geometry bounds](/nodes/GeometryBounds/documentation.md)**, **[geometry size](/nodes/GeometrySize/documentation.md)**, and **[geometry center](/nodes/GeometryCentroid/documentation.md)**, or work directly from the *points* outputs of the **[geometry input](/nodes/GeometryInputV2/documentation.md)** node.
...
- The **[geometry input](/nodes/GeometryInputV2/documentation.md)** node must be paramaterized to make the Live Component an AutoFit Live Component.
- It is up to the author to make sure the graph resolves sensibly for as many different geometry inputs as possible. Things worth handling explicitly:
  - Nothing supplied at all — the **[geometry input](/nodes/GeometryInputV2/documentation.md)** node's default is a flat `1000` unit square polyline (not truly empty geometry, despite its `emptyPolyLine` id), so a graph should still produce something reasonable before it has been given a face in SketchUp.
...
- AutoFit and a [size frame](/concepts/GeneralConcepts/sizeFrame.md) can both be present in the same Live Component, but be deliberate about it — if a size frame drives the same dimensions the incoming geometry does, the two will fight. Instead only use the sizeframe to adjust dimensions not controlled by the **[geometry input](/nodes/GeometryInputV2/documentation.md)** , or have a toggle to switch between the LC using the size frame or the geometry input.
